...
function generateImageMetadata(category: string, index: number) {
  ...
  return {
    ...
    location:
      locations[category as keyof typeof locations][
        index % locations[category as keyof typeof locations].length
      ],
  };
}
...
function generateTags(category: string, index: number) {
  ...
  return JSON.stringify(
    tagSets[category as keyof typeof tagSets][
      index % tagSets[category as keyof typeof tagSets].length
    ]
  );
}
...
async function main() {
  ...
  if (portfolioImages.length === 0) {
    console.log(
      '⚠️  No images found in public/images/portfolio/ - skipping portfolio item creation'
    );
  } else {
    console.log(
      `📸 Creating portfolio items from ${portfolioImages.length} images...`
    );
    ...
    for (let i = 0; i < portfolioImages.length; i++) {
      ...
      publishDate.setMonth(
        publishDate.getMonth() - Math.floor(Math.random() * 6)
      );
      ...
      console.log(
        `✅ Created portfolio item: ${created.title} (${categorySlug})`
      );
    }
  }
  ...
  const inquiries = [
    {
      ...
      message:
        'Hi Kilian, I am interested in joining one of your nature photography workshops. Could you please send me information about upcoming dates and pricing?',
    ...
    },
    {
      ...
      message:
        'We are looking for a videographer for our annual company presentation. The project would involve filming our facilities and interviewing key team members.',
