...
import { StorageManager, FileMetadata, ProcessedFile, storageManager } from './storage'
...
export class ImageProcessor {
  ...
  constructor(storage: StorageManager, options: Partial<ImageProcessingOptions> = {}) {
   ...
   */
  async getImageInfo(buffer: Buffer): Promise<ImageDimensions & { format: string; hasAlpha: boolean }> {
   ...
   */
  async optimizeImage(buffer: Buffer, options?: Partial<ImageProcessingOptions>): Promise<Buffer> {
  ...
  async processImage(
    ...
    const originalPath = await this.storage.saveFile(optimizedBuffer, baseName, 'originals')
    ...
    try {
      ...
      result.thumbnailPath = this.storage.getPublicUrl(thumbnailName, 'thumbnails')
    } catch (error) {
  ...
  async processImages(
    ...
    const results = await Promise.allSettled(
      files.map(file => 
        this.processImage(file.buffer, file.originalName, file.mimeType, options)
      )
    )
    ...
    results.forEach((result, index) => {
      if (result.status === 'fulfilled') {
        ...
        errors.push(`Failed to process ${files[index].originalName}: ${result.reason.message}`)
      }
    })
  ...
  async generateResponsiveSizes(
    ...
    const baseNameWithoutExt = this.storage.generateFileName(originalName).replace(/\.[^.]+$/, '')
