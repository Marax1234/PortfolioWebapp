...
async function seedAnalyticsData() {
  ...
  try {
    ...
    if (portfolioItems.length === 0) {
      console.log(
        '❌ No portfolio items found. Please seed portfolio data first.'
      );
      return;
    }
    ...
    for (let i = 0; i < totalEvents; i++) {
      ...
      if (eventType === 'portfolio_item_view') {
        ...
        const popularItems = portfolioItems.slice(
          0,
          Math.ceil(portfolioItems.length * 0.3)
        );
        const regularItems = portfolioItems.slice(
          Math.ceil(portfolioItems.length * 0.3)
        );

        if (Math.random() < 0.7 && popularItems.length > 0) {
          portfolioItemId =
            popularItems[randomInt(0, popularItems.length - 1)].id;
        } else if (regularItems.length > 0) {
          portfolioItemId =
            regularItems[randomInt(0, regularItems.length - 1)].id;
        }
    ...
    for (let i = 0; i < events.length; i += batchSize) {
      ...
      if (i % 1000 === 0) {
        console.log(
          `💾 Inserted ${Math.min(i + batchSize, events.length)}/${events.length} events...`
        );
      }
    }
    ...
    const sampleInquiries = [
      {
        ...
        message:
          'Hallo, ich interessiere mich für Ihre Hochzeitsfotografie. Könnten Sie mir ein Angebot für Juni 2024 machen?',
        inquiryType: 'WEDDING',
      },
      {
        name: 'Lisa Schmidt',
        ...
        message:
          'Ich würde gerne ein professionelles Portrait shooting buchen.',
