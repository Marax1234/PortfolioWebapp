...
import { setupAllMocks, resetAllMocks } from '@/__mocks__/setup'
...
describe('NextAuth Configuration', () => {
  ...
  describe('Credentials Authorization', () => {
    it('should reject authorization with missing credentials', async () => {
      const provider = authOptions.providers[0] as any
      const mockReq = { headers: { 'x-forwarded-for': '127.0.0.1', 'user-agent': 'test' } }
    ...
    it('should handle authentication service errors', async () => {
      ...
      const mockUserServiceInstance = {
        authenticateUser: jest.fn().mockRejectedValue(new Error('Database error')),
      }
      mockUserService.mockImplementation(() => mockUserServiceInstance)
  ...
  describe('JWT Callback', () => {
    it('should populate token with user data on first sign in', async () => {
      ...
      const result = await authOptions.callbacks!.jwt!({ token, user: mockUser } as any)
