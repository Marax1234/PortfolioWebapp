...
export const usePortfolioStore = create<PortfolioState>()(
  devtools(
    (set, get) => ({
      ...
      setLoadingMore: (loading) => set({ isLoadingMore: loading }, false, 'setLoadingMore'),
      ...
      // Lightbox Actions
      openLightbox: (index) => 
        set({ lightboxOpen: true, lightboxIndex: index }, false, 'openLightbox'),
      ...
      nextImage: () => {
        ...
        const nextIndex = lightboxIndex < items.length - 1 ? lightboxIndex + 1 : 0
        set({ lightboxIndex: nextIndex }, false, 'nextImage')
      },
      
      prevImage: () => {
        const { items, lightboxIndex } = get()
        const prevIndex = lightboxIndex > 0 ? lightboxIndex - 1 : items.length - 1
