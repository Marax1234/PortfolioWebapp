...
export class RequestLogger {
  ...
  static logRequest(context: RequestContext) {
    const { requestId, method, pathname, ip, userAgent, searchParams, size } = context
   ...
   */
  static logResponse(context: RequestContext, response: NextResponse, error?: Error) {
    ...
    if (responseTime > 500) { // Log performance for requests > 500ms
  ...
  static logSecurityEvent(
    ...
    eventType: 'UNAUTHORIZED_ACCESS' | 'PERMISSION_DENIED' | 'SUSPICIOUS_ACTIVITY' | 'RATE_LIMITED',
