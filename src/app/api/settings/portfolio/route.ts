...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    const session = await getServerSession(authOptions)
    if (!session?.user) {
      Logger.warn('Unauthorized portfolio settings update attempt', { requestId })
      return NextResponse.json(
        { error: 'Nicht authentifiziert' },
        { status: 401 }
      )
    ...
    if (validatedData.featuredItemsLimit < publishedCount) {
      Logger.warn('Portfolio settings update attempted with featured limit below current count', {
        requestId,
        userId: session.user.id,
