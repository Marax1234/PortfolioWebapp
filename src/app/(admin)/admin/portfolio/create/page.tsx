...
const createPortfolioFormSchema = z
  .object({
    title: z
      .string()
  ...
  const onSubmit = async (formData: CreatePortfolioFormData) => {
    try {
      ...
      if (processedFiles.length === 0) {
        setError(
          'Bitte laden Sie mindestens eine Datei hoch, bevor Sie das Portfolio-Element erstellen.'
        );
        setIsSaving(false);
        return;
      }
      ...
      for (let i = 0; i < processedFiles.length; i++) {
        ...
        const itemTitle =
          i === 0 ? formData.title : `${formData.title} ${i + 1}`;
        ...
        const portfolioData = {
          ...
          metadata: {
            ...
            dimensions:
              processedFile.original.width && processedFile.original.height
                ? {
                    width: processedFile.original.width,
                    height: processedFile.original.height,
                  }
                : null,
          },
          sortOrder: i,
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to create portfolio item: ${response.statusText}`
          );
        }
      ...
      console.log(
        `Successfully created ${createdItems.length} portfolio items`
      );
      router.push('/admin/portfolio');
    } catch (err) {
      setError(
        err instanceof Error ? err.message : 'Failed to create portfolio items'
      );
              ...
              <div className='space-y-6 md:col-span-2'>
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Laden Sie Bilder oder Videos für Ihr Portfolio hoch. Sie
                      können mehrere Dateien gleichzeitig hochladen.
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <FileUpload
                    ...
                    {!hasUploadedFiles && (
                      <FormHint>
                        <strong>Hinweis:</strong> Sie müssen mindestens eine
                        Datei hochladen, bevor Sie das Portfolio-Element
                        erstellen können.
                      </FormHint>
                    )}
                  </CardContent>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>
                            3-100 Zeichen. Aussagekräftiger Name für Ihr
                            Portfolio-Element
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>
                            Optionale Beschreibung (max. 500 Zeichen), die mit
                            Ihrem Portfolio-Element angezeigt wird
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormDescription>
                            Wählen Sie eine passende Kategorie für Ihr
                            Portfolio-Element aus
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                ...
                <div className='flex items-center justify-end space-x-4'>
                  ...
                  <Button
                    type='submit'
                    disabled={isSaving || !hasUploadedFiles}
                  >
                    {isSaving ? (
                      <>
                        ...
                        {processedFiles.map((file, index) => (
                          <div key={index} className='rounded-lg border p-3'>
                            ...
                            <div className='space-y-1 text-xs text-slate-600'>
                              ...
                              <p>
                                <strong>Size:</strong>{' '}
                                {Math.round(file.original.size / 1024)}KB
                              </p>
